...
type PlannerService interface {
	...
	WithCredentials(user *auth.User) error
}
...
func DefaultPlannerService() (*plannerService, error) {
	return CustomPlannerService(UserAuth(DefaultUsername, DefaultOrganization))
}

// CustomPlannerService initializes the planner service with custom *auth.User credentials
func CustomPlannerService(cred *auth.User) (*plannerService, error) {
...
// WithCredentials sets the plannerService's credentials and initializes a new Service API instance
// if the provided credentials differ from the currently stored ones.
func (s *plannerService) WithCredentials(cred *auth.User) error {
	if s.credentials.Organization == cred.Organization &&
		s.credentials.Username == cred.Username {
		return nil
	}
	newApi, err := NewServiceApi(cred)
	if err != nil {
