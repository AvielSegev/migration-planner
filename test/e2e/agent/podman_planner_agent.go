...
func NewPlannerAgentPodman(sourceID uuid.UUID, imagePath, dataDir, persistentDataDir, configDir string) (PlannerAgent, error) {
	...
	return &plannerAgentPodman{
		...
		configDir:                configDir,
		MountedDataDir:           path.Join(fmt.Sprintf(e2e.MountBasePath, agentUUID), "data"),
		MountedPersistentDataDir: path.Join(fmt.Sprintf(e2e.MountBasePath, agentUUID), "persistent-data"),
		MountedConfigDir:         path.Join(fmt.Sprintf(e2e.MountBasePath, agentUUID), "config"),
		sourceID:                 sourceID,
		agentUuid:                agentUUID,
		config: agentConfig.Config{
			ConfigDir:         configDir,
...
func (p *plannerAgentPodman) prepare() error {
	...
	if err := os.WriteFile(filepath.Join(p.MountedPersistentDataDir, "agent_id"), []byte(p.agentUuid.String()), 0644); err != nil {
...
func (p *plannerAgentPodman) spec() (*specgen.SpecGenerator, error) {
	...
	spec.Name = fmt.Sprintf("planner-agent-e2e-%s", p.agentUuid.String())
...
func (p *plannerAgentPodman) Remove() error {
	...
	if err := os.RemoveAll(fmt.Sprintf(e2e.MountBasePath, p.agentUuid)); err != nil {
