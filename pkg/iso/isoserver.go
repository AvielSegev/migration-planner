...
func (i *ServerDownloader) Get(ctx context.Context, dst io.Writer) error {
	...
	IsoURL := path.Join(i.baseServerURL, isoserver.GetIsoEndpoint)
	...
	if err != nil {
		return fmt.Errorf("failed to fetch ISO from server %q: %w", IsoURL, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("ISO server returned status %d for %q", resp.StatusCode, IsoURL)
	}
...
func (i *ServerDownloader) HealthCheck(ctx context.Context) error {
	...
	healthURL := path.Join(i.baseServerURL, isoserver.HealthEndpoint)
