...
// portForwardCommand establishes a `kubectl port-forward` command to expose a given service or deployment
...
func (o *E2ETestOptions) portForwardCommand(dest string, port string) error {
	...
	fixCommand := fmt.Sprintf("kubectl port-forward --address 0.0.0.0 %s %s:%s > /dev/null 2>&1 &", dest, port, port)
...
func (o *E2ETestOptions) ensureFullConnectivity() error {
	...
	if err := o.portForwardCommand("service/migration-planner", defaultPlannerServicePort); err != nil {
		return fmt.Errorf("error establishing connection to port %s: %v", defaultPlannerServicePort, err)
	}

	if err := o.portForwardCommand("service/migration-planner-image", defaultImagePort); err != nil {
		return fmt.Errorf("error establishing connection to port %s: %v", defaultImagePort, err)
	}

	if err := o.portForwardCommand("service/migration-planner-agent", defaultAgentPort); err != nil {
		return fmt.Errorf("error establishing connection to port %s: %v", defaultAgentPort, err)
	}

	if err := o.portForwardCommand("deploy/registry", defaultRegistryPort); err != nil {
		return fmt.Errorf("error establishing connection to port %s: %v", defaultRegistryPort, err)
	}

	if err := o.portForwardCommand("deploy/vcsim1", Vsphere1Port); err != nil {
		return fmt.Errorf("error establishing connection to port %s: %v", Vsphere1Port, err)
	}

	if err := o.portForwardCommand("deploy/vcsim2", Vsphere2Port); err != nil {
		return fmt.Errorf("error establishing connection to port %s: %v", Vsphere2Port, err)
	}
...
func validateVmsDeletion() error {
	...
	if err != nil {
		log.Fatalf("[CLI] Failed to list domains: %v", err)
	}
